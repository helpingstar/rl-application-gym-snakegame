# Original Code : https://github.com/vwxyzjn/cleanrl/blob/master/cleanrl/ppo.py
import argparse
import os
import random
import time
...
def parse_args():
    ...
    parser.add_argument("--wandb-project-name", type=str, default="gym_snakegame_size12",
    ...
    # reward 
    parser.add_argument("--blank-reward", type=float, default=-0.1,
        help="The reward for reaching an empty space.")
    parser.add_argument("--reward-scale", type=float, default=5,
        help="Size of reward for dying or getting an item")
...
def make_env(env_id, seed, idx, capture_video, run_name):
    def thunk():
        if capture_video and idx == 0:
            env = gym.make(env_id, board_size=args.board_size, render_mode="rgb_array")
        else:
            env = gym.make(env_id, board_size=args.board_size)
        
        env = ReshapeObservationV0(env, (1, args.board_size, args.board_size))
        env = DtypeObservationV0(env, np.float32)
        env = LambdaObservationV0(env, lambda obs: obs / env.unwrapped.ITEM, observation_space=gym.spaces.Box(0, 1, shape=(1, env.unwrapped.board_size, env.unwrapped.board_size), dtype=np.float32))
...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.network = nn.Sequential(
            layer_init(nn.Conv2d(1, 32, 3)),
            nn.ReLU(),
            layer_init(nn.Conv2d(32, 64, 3)),
            nn.ReLU(),
            layer_init(nn.Conv2d(64, 128, 3)),
            nn.ReLU(),
            nn.Flatten(),
            layer_init(nn.Linear(128 * 6 * 6, 2048)),
            nn.ReLU(),
        )
        ...
        for step in range(0, args.num_steps):
            ...
            for info in infos["final_info"]:
                ...
                if charts_count % args.log_charts_interval == 0:
                    ...
                    writer.add_scalar("charts/snake_length", info["snake_length"], global_step)
                    charts_count = 1
                else:
                    charts_count += 1
